...
export function SettingWorkspace() {
  ...
  async function handleSelectWorkspace() {
    try {
      ...
      console.error('选择工作区失败:', error) // EN: Failed to select workspace
    }
  }

  // 切换工作区（统一处理）
  // EN: Switch workspace (handled uniformly)
  async function switchWorkspace(path: string) {
    try {
      ...
      console.error('切换工作区失败:', error) // EN: Failed to switch workspace
    }
  }


  // 清空所有历史记录
  // EN: Clear all history entries
  async function handleClearHistory() {
    await clearWorkspaceHistory()
  }

  // 重置为默认工作区
  // EN: Reset to default workspace
  async function handleResetWorkspace() {
    try {
      // 确保默认目录存在
      // EN: Ensure default directory exists
      ...
      console.error('重置工作区失败:', error) // EN: Failed to reset workspace
    }
  }
